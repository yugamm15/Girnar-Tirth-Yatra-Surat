...
export const AuthView = ({ onBack, initialView = 'login' }) => {
  ...
  const loadUpashrayReports = async () => {
    try {
      ...
      setUpashrays(prev => prev.map(u => ({
        ...u,
        reports: reportsByUpashray[u.id] || []
      })));
    } catch (e) { console.error(e); }
  };
  ...
  const loadAdminData = async () => {
    ...
    // Only load upashrays initially for the default tab
    await loadUpashrays();
  ...
  const loadMemberData = async () => {
    ...
    await loadUpashrays(); // Members also start with upashray reports
    setIsLoadingData(false);
  };
  ...
  const deleteExistingMedia = async (mediaId, category) => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        [fieldName]: prev[fieldName].filter(m => m.id !== mediaId)
      }));
      
      // Refresh upashrays list to update thumbnails if needed
      await loadUpashrays();
    } catch (err) {
  ...
  const handleSaveUpashray = async (e) => {
    ...
    try {
      ...
      const categories = [
        { field: 'beforeFiles', label: 'Before' },
        { field: 'processFiles', label: 'Process' },
        { field: 'afterFiles', label: 'After' }
      ];

      for (const cat of categories) {
        if (formData[cat.field] && formData[cat.field].length > 0) {
          setProcessingMessage(`Uploading ${cat.label} images...`);
          for (const file of formData[cat.field]) {
            await upashrayMediaDB.upload(upashrayId, file, cat.label);
          }
        }
      }

      // Instead of full reload, update local state with the saved object
      const processedUpashray = {
        ...savedUpashray,
        name: savedUpashray.name || '',
        village: savedUpashray.village || '',
        route: savedUpashray.route || '',
      ...
      if (editingId) {
        setUpashrays(prev => prev.map(u => u.id === editingId ? { ...u, ...processedUpashray } : u));
      } else {
        setUpashrays(prev => [processedUpashray, ...prev]);
      }
      
      // Still load reports in background to ensure status icons are correct
      loadUpashrayReports(); 
      resetForm();
    } catch (err) {
      console.error('Save upashray error:', err);
      alert('Failed to save upashray');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const deleteUpashray = async (id) => {
    if (window.confirm('Are you sure you want to delete this upashray?')) {
      ...
      try {
        ...
        console.error('Delete upashray error:', err);
        setUpashrays(originalUpashrays);
        alert('Failed to delete upashray');
      }
    }
  };
  ...
  const handleSaveJinalaya = async (e) => {
    ...
    try {
      const data = {
        ...
        status: jinalayaFormData.status,
      ...
      console.error('Save jinalaya error:', err);
      alert('Failed to save jinalaya');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const startEditJinalaya = (j) => {
    ...
    setJinalayaFormData({
      ...
      status: j.status,
      beforeImg: j.beforeImg,
      processImg: j.processImg,
      afterImg: j.afterImg
    });
  ...
  const deleteJinalaya = async (id) => {
    if (window.confirm('Are you sure you want to delete this jinalaya?')) {
      ...
      try {
        ...
        console.error('Delete jinalaya error:', err);
        setJinalayas(originalJinalayas);
        alert('Failed to delete jinalaya');
      }
    }
  };
  ...
  const handleYatraDateSubmit = async (e) => {
    ...
    try {
      ...
      if (editingYatraDateId) {
        await yatraDatesDB.update(editingYatraDateId, data);
      } else {
        await yatraDatesDB.create(data);
      }
      await loadBusYatra();
      setIsYatraDateModalOpen(false);
    } catch (err) {
      console.error('Save yatra date error:', err);
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  const deleteYatraDate = async (id) => {
    if (window.confirm('Are you sure you want to delete this yatra date?')) {
      ...
      try {
        ...
        console.error('Delete yatra date error:', err);
  ...
  const openReportAndPrint = (report) => {
    window.open(`/print-report/${report.id}`, '_blank');
  };
  ...
  if (view === 'admin') {
    return (
      <AdminPanel 
        ...
        loadJinalayas={loadJinalayas}
        loadReports={loadReports}
        loadBusYatra={loadBusYatra}
  ...
  if (view === 'member') {
    return (
      <MemberPanel 
        ...
        setMemberActiveTab={(tab) => navigate(`/member/${tab}`)}
        loadedData={loadedData}
        loadReports={loadReports}
        loadUpashrayReports={loadUpashrayReports}
        isMobileMenuOpen={isMobileMenuOpen}
        setIsMobileMenuOpen={setIsMobileMenuOpen}
        handleLogout={handleLogout}
        upashrays={upashrays}
